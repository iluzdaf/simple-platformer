...
- Generic grid A* with separate flying and platformer navigation policies.
...
for the flying NPC to follow ordinary four-direction A* paths while still respecting
walls.
...
- A ground creature that patrols, uses platformer A*, jumps, chases, and bites.
- A flying creature that patrols, uses ordinary four-direction grid A*, chases, and
  bites.
...
The generic A* implementation works on `GridPosition` and receives a neighbour
function. It has no knowledge of tiles, actors, or platformer physics.
...
Platformer jumping is an isolated advanced subsystem. A small catalogue of jump arcs
is generated from `PlatformerMovementConfig`. For a candidate start cell, navigation
translates sampled actor-sized AABBs along an arc, rejects obstructed samples, and
accepts landings on standable cells. Neighbours are generated on demand rather than
stored as an editor-visible graph.

Every accepted arc is tested by replaying its input program through the real movement
and collision systems and confirming the predicted landing cell. This guards against
navigation and runtime movement drifting apart.
...
- generic A*, flying neighbours, standability, falls, jump clearance, and runtime
  replay of accepted jump arcs;
...
7. **NPC behaviour and basic A***: sensing, line of sight, target memory, two-point
   patrols, concrete NPC FSM, generic A*, flying movement and navigation, path follower,
   unreachable-path retry, and intention-driven NPC movement and attacks.
...
- A flying NPC demonstrates ordinary grid A*, patrols, detects and remembers the
