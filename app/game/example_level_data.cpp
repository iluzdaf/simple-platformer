...
namespace
{
    ...
    simple_platformer::GridPosition gridPosition(
        const Json& value,
        std::string_view sourceName,
        std::string_view path)
    {
        ...
        std::string_view path)
    {
        ...
        if ((cell == object.end()) == (feet == object.end()))
        {
            fail(
                sourceName,
                path,
                "supply exactly one of '" + std::string(cellKey) + "' or '" + std::string(feetKey) +
                    "'");
        }
        if (cell != object.end())
        {
            return simple_platformer::navigationFeet(
                gridPosition(*cell, sourceName, std::string(path) + "." + std::string(cellKey)));
        }
        return vector2(*feet, sourceName, std::string(path) + "." + std::string(feetKey));
    }

    simple_platformer::ItemId itemId(
        const Json& value,
        std::string_view sourceName,
        std::string_view path)
    {
        ...
        if (name == "coin")
        {
            return simple_platformer::Coin;
        }
        if (name == "health_potion")
        {
            return simple_platformer::HealthPotion;
        }
        if (name == "key")
        {
            return simple_platformer::Key;
        }
        fail(sourceName, path, "unknown item '" + name + "'");
    }

    simple_platformer::ExampleActorType actorType(
        const Json& value,
        std::string_view sourceName,
        std::string_view path)
    {
        ...
        if (name == "zombie")
        {
            return simple_platformer::ExampleActorType::Zombie;
        }
        if (name == "bat")
        {
            return simple_platformer::ExampleActorType::Bat;
        }
        if (name == "zombie_soldier")
        {
            return simple_platformer::ExampleActorType::ZombieSoldier;
        }
    ...
    simple_platformer::ExampleActorPlacement actor(
        const Json& value,
        std::string_view sourceName,
        std::size_t index)
    {
        const std::string path = "actors[" + std::to_string(index) + "]";
        simple_platformer::ExampleActorPlacement result;
        result.type =
        ...
        std::size_t index)
    {
        ...
        const int quantity =
            integer(member(value, "quantity", sourceName, path), sourceName, path + ".quantity");
        if (quantity <= 0)
        {
    ...
    simple_platformer::ExampleExitPlacement levelExit(
        const Json& value,
        std::string_view sourceName)
    {
        constexpr std::string_view Path = "exit";
        simple_platformer::ExampleExitPlacement result;
        result.spawnFeet = feetPosition(value, "spawnCell", "spawnFeet", sourceName, Path);
        ...
        if (const auto found = value.find("requirement"); found != value.end())
        {
            ...
            result.requirement = simple_platformer::ItemStack{
                itemId(
    ...
    std::vector<std::string> mapRows(const Json& value, std::string_view sourceName)
    {
        ...
        for (std::size_t rowIndex = 0; rowIndex < value.size(); ++rowIndex)
        {
            ...
            for (const char symbol : row)
            {
                if (symbol != '.' && symbol != '#')
                {
                    fail(sourceName, path, "only '.' and '#' tiles are supported");
                }
            }
