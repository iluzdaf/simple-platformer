...
TEST_CASE("A solid tile cast reports the earliest tile", "[physics][segment][tile]")
{
    ...
    const std::optional<float> hit =
        simple_platformer::segmentCastSolidTiles(map, {0.0F, 24.0F}, {80.0F, 24.0F});
...
TEST_CASE("A solid tile cast accounts for the moving box size", "[physics][segment][tile]")
{
    ...
    REQUIRE_FALSE(simple_platformer::segmentCastSolidTiles(map, start, end));
    REQUIRE(simple_platformer::segmentCastSolidTiles(map, start, end, {4.0F, 16.0F}).has_value());
}
...
TEST_CASE("Solid tile casts reject an invalid moving size", "[physics][segment][tile]")
{
    ...
    REQUIRE_THROWS_AS(
        simple_platformer::segmentCastSolidTiles(map, {0.0F, 0.0F}, {16.0F, 0.0F}, {-1.0F, 0.0F}),
