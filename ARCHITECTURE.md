...
The generic weighted A* implementation works on `GridPosition` and receives a
neighbour function. It has no knowledge of tiles, actors, or platformer physics.
...
to perform the connection. A* preserves that information in its result rather than
returning only a list of cells:
...
walk connections join adjacent standable cells. A generated airborne program also
